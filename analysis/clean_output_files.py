...
def clean(output_og, opt_and_thresholds):
    ...
    # If this file contains both the optimal results and the threshold policies
    if opt_and_thresholds:
        ...
        assert len(output_basic.index) == 0, (
            "Number of basic threshold policy results are"
            f"{len(output_basic.index)}"
        )
        assert len(output_cost.index) == 0, (
            "Number of cost threshold policy results are"
            f"{len(output_cost.index)}"
        )

        output = output_og

        # Add the _o suffix to all columns from the optimal policy
        cols = ["Filename", "solution_cost", "overtimeFreq", "split_shiftC",
                "split_overtimeC", "split_phaseC", "split_earlyC", "solution_cost",
                "runtime"]
        output.rename(
            columns={c: c + '_o' for c in output.columns if c in cols}, inplace=True
        )
